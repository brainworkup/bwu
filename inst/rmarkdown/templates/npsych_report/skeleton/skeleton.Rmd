...
neurobehav <-
  ...
  dplyr::mutate(
    result = glue::glue(
      "{patient}'s {description} are in the {range} range
       and above those of approximately {percentile}% 
       of {pronoun} peers."
    )
  )
```
...
```{r 07-plot-narrow-social, eval=T, fig.cap = '(ref:mtr)', double_escape=T, escape=F}
```
...
range <-
  readr::read_csv(here::here(patient, "test_score_ranges.csv"),

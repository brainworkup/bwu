...
table2 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      ...
      scale ==
        "Numbers & Letters Part B Efficiency" ~ "(ref:first-name)'s performance on a complex measure of divided attention, information processing speed, and inhibition",
      TRUE ~ as.character(description)
    )
  )
```
...
vroom::vroom_write(
  table,
  here::here(patient, paste0("csv/", test, ".csv")),
  delim = ",",
  append = TRUE,
  bom = TRUE,
  col_names = TRUE,

...
params:
  ...
  table4:
    label: "NAB Memory"
    value: ["Shape Learning Immediate Recognition"]
    input: select
    multiple: yes
    choices:
...
table1 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      ...
      scale ==
        "NAB Spatial Index" ~ "A composite screening measure of `r params$first_name`'s visuoperceptual skills, attention to detail, and visuoconstructional skills",
      scale ==
...
table2 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      ...
      scale ==
        "Numbers & Letters Part B Efficiency" ~ "`r params$first_name`'s performance on a complex measure of divided attention, information processing speed, and inhibition",
      TRUE ~ as.character(description)
    )
  )
```

...
library(rmdformats)
library(tibble)
xfun::pkg_load2("stringi", "pdftools", "tabulizer", "dplyr", "vroom", "pdftables", "qpdf", "tidyverse", "magrittr", "rJava", "shiny", "tabulizerjars", "tidyr", "dplyr", "data.table", "stringr", "rlist", "parallel", "DT")
library(rlang)
library(bwu)
```
...
```{r}
file_path <- here::here(patient, "pdf", "pai.pdf")
```

This creates a file path that joins your project directory, the value stored in `patient`, "pre_csv", and "wiat4_pdf".

### 2. Writing to a File:

The `writeLines()` function is used to write text lines to a file.
The first argument should be the content (i.e., the text lines) you want to write, and the second argument should be the file path.
So, if you wanted to write some text to the file at the path we just created, you would do:
...
file <- here::here(patient, "pdf", "pai.pdf")
## extract text
...
readr::write_lines(pai_text, here::here(patient, "pai_text.qmd"), sep = "\n")
readr::write_lines(pai_text, here::here(patient, "pai_text.typ"), sep = "\n")
...
# save
readr::write_lines(pai_text_squished, here::here(patient, "pai_text_squished.md"), sep = "\n")
readr::write_lines(pai_text_squished, here::here(patient, "pai_text_squished.qmd"), sep = "\n")
readr::write_lines(pai_text_squished, here::here(patient, "pai_text_squished.typ"), sep = "\n")
```

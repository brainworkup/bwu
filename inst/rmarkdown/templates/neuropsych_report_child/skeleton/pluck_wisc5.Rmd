---
title: "Pluck Tables from WISC-V"
params:
  patient: "Biggie"
  test:
    label: Test
...
```{r wfile}
#readr::write_lines(file, here::here(patient, "pre_csv", "wisc5_pdf"))
```

```{r rfile}
#file <- readr::read_lines(here::here(patient, "pre_csv", "wisc5_pdf"))
```
...
```{r w-area}
#readr::write_rds(area, here::here(patient, "pre_csv", "area_wisc5.rds"))
```

```{r r-area}
#area <- readr::read_rds(here::here(params$patient, "pre_csv", "area_wisc5.rds"))
```
...
table1 <-
  ...
  tidytable::mutate(
    description = dplyr::case_when(
      ...
      scale == "Processing Speed (PSI)" ~ "The PSI provides an estimate of Processing Speed (Gs). Gs refers to the efficiency of cognitive processing or speed of mental activity. It involves the ability to perform simple clerical-type tasks quickly, especially when under pressure to maintain attention and concentration. The PSI of {{ score }} ({{ ci_95 }}) is classified as {{ range }} and is ranked at the {{ percentile }}th percentile, indicating performance as good as or better than {{ percentile }}% of same age peers from the general population.",
...
```{r slice}
wisc5 <- wisc5 %>% dplyr::slice(5,1,6:7,2,9,8,10,3,12,11,13:15,4,21,20)
```

## Write out final csv

```{r write}
readr::write_csv(wisc5, here::here(patient, "csv", "wisc5.csv"), col_names = TRUE, na = "")
```

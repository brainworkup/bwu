...
(ref:baserate) A *base rate* is the prevalence of score in the general population. If a score is rare, defined as a base rate of less than 10%, it is considered *clinically significant*, and that area of functioning is probably causing problems in daily life.
...
(ref:psi) The WISC-V Processing Speed Index (PSI) is primarily a measure of Processing Speed (Gs). Gs refers to the efficiency of cognitive processing or speed of mental activity. It involves the ability to perform simple clerical-type tasks quickly, especially when under pressure to maintain attention and concentration. The difference between the scores that comprise the WISC-V PSI is statistically significant and a difference of this size occurs in less than 10% of the general population which means the difference is relatively uncommon. This means that although the composite is likely a psychometrically sound estimate of Processing Speed, it may not be a good clinical summary because it may obscure an important and meaningful difference within this domain, which often occurs when one score is below average and the other score(s) is at least average relative to most people. (ref:patient)'s WISC-V Gs-PSI score of XX (XX-XX) is classified as Low Average/Within Normal Limits and is ranked at the XXrd percentile, indicating performance as good as or better than XX% of same age peers from the general population.

(ref:vci) The WISC-V Verbal Comprehension Index (VCI) is primarily a measure of Comprehension and Knowledge (Gc). Gc refers to an individual's knowledge base (or general fund of information) that develops as a result of exposure to language, culture, general life experiences, and formal schooling. The size of the difference between the lowest and highest scores that comprise the VCI occurs in at least 10% of the general population, which makes the difference relatively common. This means that the WISC-V VCI is a good psychometric summary of Comprehension and Knowledge. Additionally, information regarding where the subtest scores fall relative to each other and relative to most people may not add clinically relevant information above and beyond the WISC-V VCI. The individual's Gc-VCI score of XXX (XXX-XXX) is classified as Upper Extreme/Normative Strength and is ranked at the XXth percentile, indicating performance as good as or better than XX% of same age peers in the general population.

(ref:fri) The WISC-V Gf - Nonverbal Fluid Reasoning Index (FRI) is primarily a measure of Fluid Reasoning (Gf). Gf refers to a type of thinking that an individual may use when faced with a relatively new or novel task that cannot be performed automatically. The individual's Gf-FRI score of 126 (121-131) is classified as Above Average/Normative Strength and is ranked at the 96th percentile, indicating performance as good as or better than 96% of same age peers from the general population.

(ref:vsi) The WISC-V Gv - Visual Processing Index (VSI) is primarily a measure of Visual Processing (Gv). Gv Refers to an individuals ability to generate visual images and perceive and analyze visual patterns and visual information. The individual's Gv-VSI score of 100 (95-105) is classified as Average/Within Normal Limits and is ranked at the 50th percentile, indicating performance as good as or better than 50% of same age peers from the general population.
...
(ref:age) `r bwu::calc_age(params$dob, params$doe2)`
...
```{r 02-glue-iq, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-academics, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-verbal, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-spatial, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-executive, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-memory, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-motor, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 02-glue-social, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 06-plot-subdomain-social, eval=F, fig.cap = '(ref:soc-cog)', double_escape=T, escape=F}
```
...
```{r 07-plot-narrow-social, eval=F, fig.cap = '(ref:soc-cog)', double_escape=T, escape=F}
```
...
```{r 02-glue-adhd, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
```{r 06-plot-adhd-adhd, eval=T, double_escape=T, escape=F, fig.cap='(ref:adhd-ef)'}
```
...
```{r 07-df-adhd, eval=T, double_escape=T, escape=F}
```
...
```{r 08-plot-executive-adhd, eval=T, double_escape=T, escape=F, fig.cap='(ref:att-ef)'}
```
...
```{r 02-glue-emotion, eval=T, double_escape=T, escape=F, cache=TRUE, cache.path='cache/'}
```
...
range <- readr::read_csv(here::here(patient, "test_score_ranges.csv"), show_col_types = FALSE)
gt::gt(range) %>%

...
neurocog <-
  ...
  dplyr::mutate(
    result = glue::glue(
      "{patient}'s ability to {description} as measured by the {scale} subtest was {range} relative to same age peers."
    )
  )
```
...
neurobehav <-
  ...
  dplyr::mutate(
    result = glue::glue(
      "{patient}'s score on {scale} (i.e., {description}) was {range}."
    )
  )
```

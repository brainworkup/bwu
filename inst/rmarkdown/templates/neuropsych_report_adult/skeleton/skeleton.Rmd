...
header:
  - pos: ro
    next: "`r params$lastname`, `r params$firstname`"
caption:
  - labelfont=bf
  - labelsep=period
  - font=small
...
## Plots
  eval_emotion_plot1:
    label: "Emotional/Behavioral/Personality Plot 1"
    value: FALSE
  eval_emotion_plot2:
    label: "Emotional/Behavioral/Personality Plot 2"
    value: TRUE
  eval_pass_plot:
    label: "PASS Plot"
    value: TRUE
...
(ref:pct) A *percentile* shows `r params$firstname`'s standing relative to peers. For example, a percentile rank of 70 means `r params$firstname` performed as well as or better than 70% of individuals the same age.
...
(ref:first-name) `r params$firstname`

(ref:last-name) `r params$lastname`
...
(ref:patient) `r params$firstname` `r params$lastname`
...
```{r nse, results = 'asis', cache = FALSE}
cat(readLines("2.0_nse.md"), sep = "\n")
```
...
```{r nt-obs, results = 'asis', cache = FALSE}
...
```{r, make-factors-neurocog, cache = FALSE}
...
```{r, make-factors-neurobehav, cache = FALSE}
...
(ref:unstructured-task) This was a novel planning task published as part of the NIH EXAMINER Executive Function Battery [@kramer2014]. The task allows the examinee to choose which problems they want to work on to maximize points earned. In doing so, examinees need to plan ahead, avoid items that are strategically poor choices, and be cognizant of when a particular booklet offers diminishing returns. (ref:firstname) performed this task effortlessly.
...
(ref:domain) *Key Findings* General intellectual functioning is a strength. Verbal skills are significantly stronger than nonverbal/spatial processing skills. Attentional fluency (i.e., working memory and processing speed) and abstract reasoning are areas of weakness. Notably, even `r params$firstname`'s normatively average scores reflect performances that are well below cognitive expectations for `r params$him_her`. *Note:* *z* = 0 (mean) ± 1 (s.d.)
...
```{r domain-plot, fig.cap = '(ref:domain)', fig.height = 3.5, cache = FALSE}
bwu::dotplot(
  data = domain,
  x = domain$z_mean_dom,
  y = domain$domain,
...
(ref:timed) **Timed vs. Untimed Tests** `r params$firstname` performs more than a half of a standard deviation better on tasks that are untimed compared to tasks that require `r params$him_her` to respond both accurately and quickly.
...
```{r, sirf, results = 'asis', cache = FALSE}
cat(readLines("3.0_sirf.md"), sep = "\n")
```
...
```{r, recs, results = 'asis', cache = FALSE}
cat(readLines("4.0_recs.md"), sep = "\n")
```

It was a pleasure to work with `r params$firstname`.

...
```{r 06-plot-subdomain-iq, eval = TRUE, fig.cap = '(ref:g) (ref:gc) (ref:gf)'}
```
...
```{r 06-plot-subdomain-academics, eval=T, fig.cap = '(ref:acad)'}
```
...
```{r 07-plot-narrow-verbal, eval=T, fig.cap = '(ref:vrb)'}
```
...
```{r 02-glue-spatial, cache=TRUE, cache.path='cache/'}
```

```{r 03-table-spatial}
```

```{r 04-kable-spatial, double_escape=T, escape=F, fig.fullwidth = TRUE}
```
...
```{r 07-plot-narrow-spatial, eval=T, fig.cap = '(ref:spt)'}
```
...
```{r 02-glue-executive, cache = TRUE, cache.path = 'cache/'}
```


```{r 03-table-executive}
```

```{r 04-kable-executive, double_escape=T, escape=F, fig.fullwidth = TRUE}
```

```{r 05-df-executive}
```

```{r 06-plot-subdomain-executive, eval=T, fig.cap = '(ref:att-exe)'}
```
...
```{r 04-kable-memory, double_escape=T, escape=F, fig.fullwidth = TRUE}
```
...
```{r 04-kable-motor, double_escape=T, escape=F, fig.fullwidth = TRUE}
```
...
```{r 04-kable-social, double_escape=T, escape=F, fig.fullwidth = TRUE}
```
...
```{r 06-plot-adhd-adhd, eval = TRUE, fig.cap='(ref:adhd-ef)'}
```

```{r 07-df-adhd}
```

```{r 08-plot-executive-adhd, eval = TRUE, fig.cap='(ref:att-ef)'}
```

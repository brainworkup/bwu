...
(ref:gcf) *General ability* refers to an overall capacity to reason, to solve
problems, and to learn useful information. *Crystallized knowledge* refers to
the ability to learn and use language to reason and understand how the world
works. *Fluid reasoning* refers to the ability to use logical reasoning to
figure things out without being told exactly how things work, analyze and solve
novel problems, identify patterns and relationships that underpin these
problems, and apply logic.
...
relation to normative expectations and the magnitude of clinical severity. Test
scores are easiest to interpret as percentiles[^ref-1], which are provided in
the tables below, and as *z*-scores (M = 0, SD = 1)[^ref-2], which are used to
plot the various test and domain scores below.
...
```{r gauss-plot-broad, eval = FALSE, fig.cap='(ref:gauss-plot-broad)', fig.width = 6, fig.height = 5, fig.pos = '!ht', cache = TRUE}
knitr::include_graphics("plot_broad.png", auto_pdf = TRUE)
```

```{r read-csv, include = FALSE}
library(tidyverse)
data_path <- here::here("Reports", patient, "csv")
...
```{r, write-neuro-data}
readr::write_csv(neuropsych, here::here("Reports", patient, "neuropsych.csv"))
readr::write_csv(neurocog, here::here("Reports", patient, "neurocog.csv"))
readr::write_csv(neurobehav, here::here("Reports", patient, "neurobehav.csv"))
readr::write_csv(validity, here::here("Reports", patient, "validity.csv"))
```

```{r, read-neuro-data}
neuropsych <- readr::read_csv(here::here("Reports", patient, "neuropsych.csv"))
neurocog <- readr::read_csv(here::here("Reports", patient, "neurocog.csv"))
neurobehav <- readr::read_csv(here::here("Reports", patient, "neurobehav.csv"))
validity <- readr::read_csv(here::here("Reports", patient, "validity.csv"))
```
...
```{r, eval = FALSE}
g <- bwu::gpluck_get_index_scores("Reports", patient = patient)
```
...
knitr::read_chunk(here::here("Reports", patient, "02.02_iq.R"))
```
...
```{r 06-plot-subdomain-iq, eval = FALSE, fig.cap = '(ref:g) (ref:gc) (ref:gf)'}
```
...
knitr::read_chunk(here::here("Reports", patient, "02.03_academics.R"))
```
...
knitr::read_chunk(here::here("Reports", patient, "02.04_verbal.R"))
```
...
```{r 06-plot-subdomain-verbal, eval = params$eval_verbal, fig.cap = '(ref:vrb)'}
```

```{r 07-plot-narrow-verbal, eval = F, fig.cap = '(ref:vrb)'}
```
...
knitr::read_chunk(here::here("Reports", patient, "02.05_spatial.R"))
```
...
```{r 06-plot-subdomain-spatial, eval = params$eval_spatial, fig.cap = '(ref:spt)'}
```

```{r 07-plot-narrow-spatial, eval = FALSE, fig.cap = '(ref:spt)'}
```
...
knitr::read_chunk(here::here("Reports", patient, "02.06_executive.R"))
```
...
knitr::read_chunk(here::here("Reports", patient, "02.07_memory.R"))
```
...
knitr::read_chunk(here::here("Reports", patient, "02.08_motor.R"))
```
...
knitr::read_chunk(here::here("Reports", patient, "02.09_social.R"))
```
...
(ref:caars-cefi) *Note:* On the *Conners' Adult ADHD Rating Scale (CAARS)*,
higher scores reflect reduced functioning in a given area, but on the
*Comprehensive Evaluation of Executive Functioning (CEFI)*, higher scores
reflect relative strengths in a given area.
...
knitr::read_chunk(here::here("Reports", patient, "02.10_adhd.R"))
```
...
knitr::read_chunk(here::here("Reports", patient, "02.11_emotion.R"))
```
...
```{r 03-table-axis1, eval = params$eval_emotion}
```

```{r 03-table-axis2, eval = params$eval_emotion}
```

```{r 03-table-sud, eval = params$eval_emotion}
```

```{r 03-table-social, eval = params$eval_emotion}
```
...
(ref:domain) *Key Findings* Neuropsychological test results show a
pattern of above average performance across all scales. Verbal Reasoning was
exceptionally strong, while Memory, Language, Working Memory and Psychomotor
Speed were all above average. Spatial, Planning, Nonverbal Reasoning and
Cognitive Efficiency were all scored as high average. Notably, even
`r params$first_name`'s normatively average scores for Attention Span and
Attentional Fluency reflect performances that are well below cognitive
expectations for `r params$him_her`. *Note:* *z* = 0 (mean) ± 1 (s.d.)

```{r summary-df}
summary <-
  readr::read_csv(here::here("Reports", patient, "csv", "index_scores.csv")) |>
  dplyr::filter(test_type == "npsych_test") |>
  dplyr::filter(domain == "Summary") |>
  dplyr::filter(!is.na(domain))
```

```{r summary-plot, fig.cap = '(ref:summary)', fig.height = 3.5}
bwu::dotplot(
  data = summary,
  x = summary$z,
  y = summary$scale,
...
```{r domain-df, eval=FALSE}
domain <-
  neurocog |>
  dplyr::filter(test_type == "npsych_test") |>
  dplyr::filter(domain != "Performance Validity") |>
  dplyr::filter(!is.na(domain))
```

```{r domain-plot, eval=FALSE, fig.cap = '(ref:domain)', fig.height = 3.5}
bwu::dotplot(
  data = domain,
  x = domain$z_mean_dom,
  y = domain$domain,
...
```{r sig, out.width='25%', fig.align='left', cache = TRUE}
...
range <-
  readr::read_csv(here::here(
    "Reports", patient, "test_score_ranges.csv"

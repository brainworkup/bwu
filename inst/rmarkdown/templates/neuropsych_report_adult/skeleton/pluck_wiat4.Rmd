...
plucked_table <- tabulizer::extract_tables(
  file = file,
  pages = c(12, 13),
  method = "stream",
  area = area
...
######
######## text works
plucked_tables <- tabulizer::extract_text(
  file = file,
  pages = c(12, 13)
)
...
patient <- "Nestor"
file <- fs::as_fs_path(file.choose())
...
wiat4 <-
  wiat4 %>%
  dplyr::mutate(across(everything(), function(x) ifelse(x == "NA NA NA", NA, x))) |>
  janitor::clean_names()
...
table1 <- dplyr::mutate(table1, raw_score = "", .before = V2)
...
table2 <- dplyr::mutate(table2, raw_score = NULL, .before = V2)
...
```{r select}
table1 <- table1 |> dplyr::select(all_of(params$keep))
table2 <- table2 |> dplyr::select(all_of(params$keep))

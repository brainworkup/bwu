...
neuropsych <-
  vroom::vroom(
    here::here(patient, "neuropsych.csv"), show_col_types = FALSE)
neurocog <-
  vroom::vroom(
    here::here(patient, "neurocog.csv"), show_col_types = FALSE) |>
  tidytable::filter(domain != "Effort/Validity")
neurobehav <-
  vroom::vroom(
    here::here(patient, "neurobehav.csv"), show_col_types = FALSE) |> tidytable::filter(domain != "Effort/Validity")
```
...
# 2. sort hi to lo
ncog1 <- tidytable::arrange(ncog1, tidytable::desc(zMean))
...
ncog_level2_drill <-
  lapply(unique(neurocog$domain), function(x_level) {
    ...
    # 2. sort hi to lo
    ncog2 <- tidytable::arrange(ncog2, tidytable::desc(zMean))
...
ncog_level3_drill <-
  lapply(unique(neurocog$domain), function(x_level) {
    ...
    lapply(unique(ncog2$subdomain), function(y_level) {
      ...
      ncog3 <- tidytable::arrange(ncog3, tidytable::desc(zMean))
  ...
  lapply(unique(neurocog$domain), function(x_level) {
    ...
    lapply(unique(ncog2$subdomain), function(y_level) {
      ...
      lapply(unique(ncog3$narrow), function(z_level) {
        ...
        ncog4 <- tidytable::arrange(ncog4, tidytable::desc(zMean))
...
```{r theme-merge1}
theme_merge <- highchater::hc_theme_merge(
  highchater::hc_theme_monokai(),
  highchater::hc_theme_darkunica()
)
```

```{r drilldown-plot, fig.cap='HighchartR Drilldown'}
# Tooltip
...
# 2. sort hi to lo
nbhv1 <- tidytable::arrange(nbhv1, tidytable::desc(zMean))

# 3. create tibble with new column with domain name lowercase
nbhv_level1_status <- tibble::tibble(
  name = nbhv1$domain,
  y = nbhv1$zMean,
  y2 = nbhv1$zPct,
  range = nbhv1$range,
...
nbhv_level2_drill <-
  lapply(unique(neurobehav$domain), function(x_level) {
    ...
    # 2. sort hi to lo
    nbhv2 <- tidytable::arrange(nbhv2, tidytable::desc(zMean))

    # 3. create tibble with new column with domain name lowercase
    nbhv_level2_status <- tibble::tibble(
...
nbhv_level3_drill <-
  lapply(unique(neurobehav$domain), function(x_level) {
    ...
    lapply(unique(nbhv2$subdomain), function(y_level) {
      ...
      nbhv3 <- tidytable::arrange(nbhv3, tidytable::desc(zMean))

      nbhv_level3_status <- tibble::tibble(
        name = nbhv3$narrow,
        y = nbhv3$zMean,
        y2 = nbhv3$zPct,
        range = nbhv3$range,
  ...
  lapply(unique(neurobehav$domain), function(x_level) {
    ...
    lapply(unique(nbhv2$subdomain), function(y_level) {
      ...
      lapply(unique(nbhv3$narrow), function(z_level) {
        ...
        nbhv4 <- tidytable::arrange(nbhv4, tidytable::desc(zMean))

        nbhv_level4_status <- tibble::tibble(
          name = nbhv4$scale,
          y = nbhv4$zMean,
          y2 = nbhv4$zPct,
          range = nbhv4$range
...
```{r drilldown2-plot, fig.cap='HighchartR Drilldown'}
# Tooltip
x <- c("Name", "Score", "Percentile", "Range")
y <- c("{point.name}", "{point.y}", "{point.y2}", "{point.range}")
tt <- tooltip_table(x, y)
...
## Create drilldown bar plot zscores
plot2 <-
  highchater::highchart() |>
  highchater::hc_title(
    text = patient,
    style = list(
      fontSize = "15px")) |>
  highchater::hc_add_series(
    nbhv_level1_status,
    type = "bar",
    name = "Behavioral Rating Scales",
    highchater::hcaes(x = name, y = y)) |>
  highchater::hc_xAxis(
    type = "category",
    title = list(
      text = "Domain"),
  ...
  highchater::hc_yAxis(
    ...
    labels = list(
      format = "{value}")) |>
  highchater::hc_tooltip(
    pointFormat = tt,
    useHTML = TRUE,
    valueDecimals = 1) |>
  highchater::hc_plotOptions(
    series = list(
      colorByPoint = TRUE,
      allowPointSelect = TRUE,
      dataLabels = TRUE)) |>
  highchater::hc_drilldown(
    allowPointDrilldown = TRUE,
    series = c(
      nbhv_level2_drill,
      nbhv_level3_drill,
      nbhv_level4_drill)) |>
  highchater::hc_colorAxis(
    minColor = "red",
    maxColor = "blue") |>
  highchater::hc_add_theme(
    theme_merge) |>
  highchater::hc_chart(
    style = list(
      fontFamily = "Cabin"),

...
df_validity <-
  ...
  tidytable::mutate(
    narrow = tidytable::case_when(
      ...
      # SUD
      scale == "ALC Estimated Score" ~ "Estimated Drug/Alcohol Use",
      scale == "DRG Estimated Score" ~ "Estimated Drug/Alcohol Use",
      TRUE ~ as.character(narrow)
    )
  )
```
...
```{r relocate}
pai1 <- df_clinical |> tidytable::relocate(c(raw_score, score, percentile, range,
ci_95), .before = test)

pai2 <- df_validity |> tidytable::relocate(c(raw_score, score, percentile, range,
ci_95), .before = test)

pai3 <- df_inatt |> tidytable::relocate(c(raw_score, score, percentile, range, ci_95), .before = test)

pai <- tidytable::bind_rows(
  pai1,
  pai2,
  pai3
)

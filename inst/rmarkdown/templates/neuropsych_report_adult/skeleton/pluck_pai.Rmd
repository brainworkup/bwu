...
df_validity <- plucked_table |>
  tidytable::select(ICN_T:PIM_P_T, MAL_T:HRI_T, ALC_Est:DRG_Est) |>
  tidyr::gather() |>
  tidytable::rename(scale = key, score = value)
...
adhd_items <- c(R13, R15, R18, R21, R34, R57, R94, R112, R139, R167, R169, R171, R195, R198, R201, R213, R233, R236, R254, R282, R299, R311, R322, R325)
...
df_validity[10, 1] <- c("Malingered Pain-Related Disability Discriminant
Function")
...
# Response Set Groups
df_validity[17, 1] <- c("All 'All false'")
df_validity[18, 1] <- c("All 'All mainly true'")
df_validity[19, 1] <- c("All 'All slightly true'")
df_validity[20, 1] <- c("All 'very true'")
df_validity[21, 1] <- c("Fake bad")
df_validity[22, 1] <- c("Fake good")
df_validity[23, 1] <- c("NIM predicted profile")
df_validity[24, 1] <- c("PIM predicted profile")
df_validity[25, 1] <- c("Random responding")

# adhd
df_inatt[1, 1] <- c("Inattention Index")
# ADHD scales
df_adhd[1, 1] <- c("ADHD Index")
```
...
```{r mutate}
df_clinical <- bwu::gpluck_make_columns(
  table,
  raw_score = "",
  range = "",
  ci_95 = "",
...
df_make_columns <- function(df) {
  df <- bwu::gpluck_make_columns(
    ...
    score_type = "t_score",
    absort = paste0(
      tolower(test), "_", tolower(scale), "_", seq_len(nrow(df))
    ),
    description = "",
    result = ""
  )
...
df_validity <-
  ...
  tidytable::mutate(
    subdomain = tidytable::case_when(
      ...
      scale == "Malingered Pain-Related Disability Discriminant
Function" ~ "Symptom Validity",
      # Positive Distortion Indicators
      ...
      # Response Set Groups
      scale == "All 'All false'" ~ "Symptom Validity",
      scale == "All 'All mainly true'" ~ "Symptom Validity",
      scale == "All 'All slightly true'" ~ "Symptom Validity",
      scale == "All 'very true'" ~ "Symptom Validity",
      scale == "Fake bad" ~ "Symptom Validity",
      scale == "Fake good" ~ "Symptom Validity",
      scale == "NIM predicted profile" ~ "Symptom Validity",
      scale == "PIM predicted profile" ~ "Symptom Validity",
      scale == "Random responding" ~ "Symptom Validity",
      TRUE ~ as.character(subdomain)
    )
  )
```
...
df_validity <-
  ...
  tidytable::mutate(
    narrow = tidytable::case_when(
      ...
      scale == "Malingered Pain-Related Disability Discriminant
Function" ~ "Negative Distortion Indicators",
      # Positive Distortion Indicators
...
```{r relocate}
pai <- df_clinical |> tidytable::relocate(c(raw_score, score, percentile, range,
ci_95), .before = test)
pai <- df_validity |> tidytable::relocate(c(raw_score, score, percentile, range,
ci_95), .before = test)
pai <- df_inatt |> tidytable::relocate(c(raw_score, score, percentile, range, ci_95), .before = test)
```

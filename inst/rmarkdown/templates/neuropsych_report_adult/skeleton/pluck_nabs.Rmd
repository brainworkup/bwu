...
table2 <-
  ...
  mutate(
    subdomain = case_when(
      ...
      scale %in% nl_time ~ "Processing Speed",
      scale %in% nl_errors ~ "Response Monitoring",
      TRUE ~ as.character(subdomain)
    )
  )
```
...
table3 <-
  ...
  mutate(
    subdomain = case_when(
      scale %in% "Auditory Comprehension" ~ "Comprehension",
      scale %in% "Naming" ~ "Expression",
      TRUE ~ as.character(subdomain)
    )
  )
```
...
table2 <-
  ...
  mutate(
    narrow = case_when(
      ...
      scale == "Digits Forward" ~ "Auditory Attention",
      scale == "Digits Forward Longest Span" ~ "Auditory Attention",
      scale == "Digits Backward" ~ "Verbal Working Memory",
      scale == "Digits Backward Longest Span" ~ "Verbal Working Memory",
...
table2 <-
  ...
  mutate(
    timed = case_when(
      ...
      scale %in% nl_errors ~ "Timed",
      scale %in% nl_time ~ "Timed",
      TRUE ~ as.character(timed)
    )
  )
```
...
table1 <-
  ...
  mutate(
    description = case_when(
      scale ==
        "NAB Total Index" ~ "Broad neurocognitive functioning (independent of general intelligence) across domains of attention and executive functioning, language and spatial processing, and learning and memory",
      scale ==
        "NAB Attention Index" ~ "An index of basic and complex verbal and nonverbal attentional capacity/functioning",
      scale ==
        "NAB Language Index" ~ "General expressive/receptive language skills",
      scale ==
        "NAB Memory Index" ~ "Overall learning and memory functioning",
      scale ==
        "NAB Spatial Index" ~ "Overall performance across measures of visual perception, construction, motor integration, and spatial awareness",
      scale ==
        "NAB Executive Functions Index" ~ "An index of general executive functioning",
...
table2 <-
  ...
  mutate(
    description = case_when(
      ...
      scale == "Digits Forward" ~ "The ability to register, maintain, and manipulate auditory information",
      ...
      scale == "Numbers & Letters Part A Speed" ~ "Psychomotor speed",
      scale == "Numbers & Letters Part A Errors" ~ "Response/error monitoring during a selective attention task",
      scale == "Numbers & Letters Part A Efficiency" ~ "Psychomotor speed requring fluent selective and sustained attention/concentration",
      scale == "Numbers & Letters Part B Efficiency" ~ "Cognitive efficiency requiring divided attention, rapid set-shifting, and inhibitory control",
      TRUE ~ as.character(description)
    )
  )
```
...
table3 <-
  ...
  mutate(
    description = case_when(
      ...
      scale ==
        "Naming" ~ "Visual confrontation naming/word-finding",
      TRUE ~ as.character(description)
    )
  )
```
...
table4 <-
  ...
  mutate(
    description = case_when(
      ...
      scale ==
        "Shape Learning Percent Retention" ~
        "visual memory percent retention",
      scale ==
      ...
      scale ==
        ...
        "story memory percent retention",
      TRUE ~ as.character(description)
    )
  )
```
...
```{r result2}
# if (params$domain == "attention") {}
table2 <-
  table2 |>
  dplyr::mutate(
    result = dplyr::case_when(
      scale ==
        "Orientation" ~ glue::glue("{description} was {range}/intact.\n"),
      scale ==
      ...
      scale ==
        "Numbers & Letters Part B Efficiency" ~ glue::glue("{description} was {range} relative to same-aged peers.\n"),
      TRUE ~ as.character(result)
    )
  )
```
...
table3 <-
  ...
  mutate(
    result = case_when(
      scale ==
        "Auditory Comprehension" ~ glue::glue("{description} were {range}.\n"),
      scale ==
        "Naming" ~ glue::glue("{description} was {range}, with no obvious deficits in conversational/informal expression.\n"),
      TRUE ~ as.character(result)
    )
  )
```
...
```{r result4}
# if (params$domain == "memory") {
table4 <-
  table4 |>
  dplyr::mutate(
    result = dplyr::case_when(
      scale == "Story Learning Immediate Recall" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      scale == "Story Learning Delayed Recall" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      scale == "Shape Learning Immediate Recognition" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      scale == "Shape Learning Delayed Recognition" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      TRUE ~ as.character(result)
    )
...
table5 <-
  ...
  dplyr::mutate(
    result = dplyr::case_when(
      scale == "Visual Discrimination" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      scale == "Design Construction" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      TRUE ~ as.character(result)
    )
...
```{r result6}
# if (params$domain == "executive") {
table6 <-
  table6 |>
  dplyr::mutate(
    result = dplyr::case_when(
      scale == "Mazes" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      scale == "Word Generation" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      scale == "Word Generation Perseverations" ~ glue::glue(
        "{description} was {range}.\n"
      ),
      TRUE ~ as.character(result)
    )

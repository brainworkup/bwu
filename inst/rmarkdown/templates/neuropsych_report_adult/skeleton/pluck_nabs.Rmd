  ...
  tidytable::mutate(
    description = tidytable::case_when(
      ...
      scale ==
        ...
        "Psychomotor speed requiring selective attention/concentration and vigilance",
      scale ==
...
table4 <-
  ...
  tidytable::mutate(
    description = tidytable::case_when(
      ...
      scale ==
        "Story Learning Percent Retention" ~
        "Percentage of story detail retained over time, scaled to how much was initially learned",
      TRUE ~ as.character(description)
    )
  )
```
  ...
  tidytable::mutate(
    description = tidytable::case_when(
      scale ==
        ...
        "Perceptual processing, discrimination, and pattern matching of a series of abstract images requiring close attention to visual detail",
      scale ==
        "Design Construction" ~
        "Constructing and recreating abstract visual designs from a model using flat polygons ('tans')",
      TRUE ~ as.character(description)
    )
  )
```
...
table6 <-
  ...
  tidytable::mutate(
    description = tidytable::case_when(
      scale ==
        "Mazes" ~
        "Planning, foresight, and adaptive abilities examined through a series of maze-tracing tasks",
      scale ==
        "Word Generation" ~
        "Spontaneous word retrieval fluency/generativity",
      scale ==
        "Word Generation Perseverations" ~
        "Response/error monitoring during a word generation task",
      TRUE ~ as.character(description)
    )
  )
```

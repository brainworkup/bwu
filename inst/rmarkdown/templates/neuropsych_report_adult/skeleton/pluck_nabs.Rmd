...
```{r wfile}
writeLines(file, here::here("Reports", patient, "pre_csv", "nabs_pdf"))
```

```{r, readfile, eval = FALSE}
file <- readLines(here::here("Reports", patient, "pre_csv", "nabs_pdf"))
```
...
```{r s-area}
saveRDS(area, here::here("Reports", patient, "pre_csv", "area_nabs.rds"))
```

```{r r-area, eval=FALSE}
area <- readRDS(here::here("Reports", patient, "pre_csv", "area_nabs.rds"))
```
...
table1 <-
  ...
  tidytable::mutate(
    description = tidytable::case_when(
      ...
      scale ==
        "NAB Spatial Index" ~ "A composite screening measure of (ref:first-name)'s visuoperceptual skills, attention to detail, and visuoconstructional skills",
      scale ==
...
```{r write-out}
readr::write_csv(nabs, here::here("Reports", patient, "csv", "nabs.csv"), col_names = TRUE, na = "")
...
tidytable::fwrite(
  table,
  here::here("Reports", patient, "csv", paste0(test, ".txt")),
  append = TRUE
)
```

  ...
  mutate(
    description = case_when(
      scale ==
        ...
        "Perceptual processing, discrimination, and pattern matching of visual details from a series of abstract visual images requiring close attention to visual detail",
      scale ==
        "Design Construction" ~
        "Constructing and recreating abstract visual designs from a model using flat polygons (called tans)",
      TRUE ~ as.character(description)
    )
  )
```
...
table2 <-
  ...
  dplyr::mutate(
    result = dplyr::case_when(
      scale ==
        "Orientation" ~
        glue::glue("{description} was [intact/impaired].\n"),
      scale ==
        "Digits Forward Longest Span" ~
...
table3 <-
  ...
  mutate(
    result = case_when(
      ...
      scale ==
        "Naming" ~
        glue::glue("{description} was {range}, with no obvious deficits in conversational/informal expression.\n"),
      TRUE ~ as.character(result)
    )
  )
```

...
# exclude variables
df_index <- dplyr::select(df_index, -category)
...
# exclude variables
df_att <- dplyr::select(df_att, -c(z_score, category, base_rate))
...
# exclude variables
df_lan <- dplyr::select(df_lan, -c(z_score, category, base_rate))
...
table4 <-
  dplyr::mutate(table4, z = (score - 50) / 10) %>%
  dplyr::mutate(percentile = pnorm(z) * 100)
...
table5 <-
  dplyr::mutate(table5, z = (score - 50) / 10) %>%
  dplyr::mutate(percentile = pnorm(z) * 100)
...
table6 <-
  dplyr::mutate(table6, z = (score - 50) / 10) %>%
  dplyr::mutate(percentile = pnorm(z) * 100)
...
table7 <-
  dplyr::mutate(table7, z = (score - 50) / 10) %>%
  dplyr::mutate(percentile = pnorm(z) * 100)
...
table8 <-
  dplyr::mutate(table8, z = (score - 50) / 10) %>%
  dplyr::mutate(percentile = pnorm(z) * 100)
...
# exclude variables
df_exe <- dplyr::select(df_exe, -c(z_score, category))

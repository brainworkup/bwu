...
table1 <-
  ...
  tidytable::mutate(
    description = tidytable::case_when(
      ...
      scale ==
        "Working Memory (WMI)" ~
        "Working Memory (*G*wm), which refers to the ability to hold information in immediate awareness and then manipulate or transform it in some way within a few seconds",
      scale ==
        "Processing Speed (PSI)" ~
        "Processing Speed (*G*s) performance (i.e., the efficiency of cognitive processing or speed of mental activity, especially when under pressure to maintain attention and concentration)",
  ...
  tidytable::mutate(
    result = tidytable::case_when(
      ...
      scale == "Working Memory (WMI)" ~ glue::glue(
        "{description} fell in the {range} range.^[(ref:working-memory)]\n"
      ),
      scale == "Processing Speed (PSI)" ~ glue::glue(
        "{description} was {range} and a relative strength|weakness.^[(ref:processing-speed)]\n"
      ),

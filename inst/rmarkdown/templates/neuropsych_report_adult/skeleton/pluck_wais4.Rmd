...
table1 <-
  ...
  tidytable::mutate.(
    description = tidytable::case_when.(
      ...
      scale ==
        "Verbal Comprehension (VCI)" ~
        "An estimate of Crystallized Intelligence (Gc)",
      scale ==
        "Perceptual Reasoning (PRI)" ~
        "Fluid Reasoning (Gf)",
      scale ==
        "Working Memory (WMI)" ~
        "Working Memory (Gwm), which refers to the ability to hold information in immediate awareness and then manipulate or transform it in some way within a few seconds",
      scale ==
        "Processing Speed (PSI)" ~
        "Processing Speed (Gs) performance (i.e., the efficiency of cognitive processing or speed of mental activity, especially when under pressure to maintain attention and concentration)",
...
```{r pluck7, eval = params$span}
table7 <- tibble::as_tibble(plucked_table[[7]])
colnames(table7) <- params$colnames4
table7 <- tidytable::na_if(table7, "")
table7 <- tidytable::na_if(table7, "NA")
table7 <- tidytable::na_if(table7, "-")
table7 <- tidytable::na_if(table7, "--")
table7 <- tidytable::na_if(table7, "---")
to_double <- c("raw_score", "score", "percentile")
...
```{r rownames7, eval = params$span}
table7$scale <- params$table7
```

## Variables to keep

```{r keep7, eval = params$span}
table7 <- table7 |> tidytable::select(all_of(params$keep2))
```

## Mutate columns

```{r mutate7, eval = params$span}
...
```{r relocate7, eval = params$span}
table7 <- table7 |> tidytable::relocate(c(raw_score, score, percentile, range, ci_95), .before = test)
```

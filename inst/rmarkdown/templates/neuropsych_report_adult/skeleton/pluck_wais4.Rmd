...
params:
  ...
  test_name:
    ...
    choices:
      ...
      - WISC-V
  version:
    label: "Full (8 subtest) or Brief (4 subtest) version"
    value: brief
    input: select
    multiple: no
    choices:
...
```{r mutate3, eval = params$eval_vci}
...
```{r narrow3, eval = params$eval_vci}
table3 <-
  table3 |>
  tidytable::mutate(
    narrow = tidytable::case_when(
...
```{r result3, eval = params$eval_vci}
table3 <-
  table3 %>%
  tidytable::mutate(
    result = glue::glue(
...
```{r mutate4, eval = params$eval_vci}
...
```{r narrow4, eval = params$eval_vci}
table4 <-
  table4 |>
  tidytable::mutate(
    narrow = tidytable::case_when(
...
```{r result4, eval = params$eval_vci}
table4 <-
  table4 %>%
  tidytable::mutate(
    result = glue::glue(
...
```{r mutate5, eval = params$eval_vci}
...
```{r narrow5, eval = params$eval_vci}
table5 <-
  table5 |>
  tidytable::mutate(
    narrow = tidytable::case_when(
...
```{r result5, eval = params$eval_vci}
table5 <-
  table5 %>%
  tidytable::mutate(
    result = glue::glue(
...
```{r relocate5, eval = params$eval_vci}
...
tidytable::fwrite(
  table,
  here::here(patient, "csv", paste0(test, ".csv")), # might not work
  append = TRUE
)
```

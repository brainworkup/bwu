...
table1 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      ...
      scale == "Working Memory (WMI)" ~ "Working Memory (Gwm), which refers to the ability to hold information in immediate awareness and then manipulate or transform it in some way within a few seconds",
...
table2 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      scale == "Similarities" ~ "Verbal inductive reasoning",
      scale == "Vocabulary" ~ "Word/lexical knowledge",
...
table2 <-
  ...
  dplyr::mutate(
    result = glue::glue(
      ...
      TRUE ~ as.character(result) # this might need to be deleted!!!!
    )
  )
```
...
table3 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      scale == "Block Design" ~ "Understanding visual-spatial relationships to construct geometric designs from a model",
...
```{r rownames6, eval = params$eval_tb6}
table6$scale <- params$table6
```

## Select variables to keep

```{r keep6, eval = params$eval_tb6}
table6 <- table6 %>% dplyr::select(all_of(params$keep2))
```

## Mutate columns

```{r mutate6, eval = params$eval_tb6}
...
```{r result6, eval = params$eval_tb6}
# result text
table6 <-
  table6 %>%
  dplyr::mutate(
...
```{r relocate6, eval = params$eval_tb6}
...
```{r rownames7, eval = params$eval_tb7}
table7$scale <- params$table7
```

## Variables to keep

```{r keep7, eval = params$eval_tb7}
table7 <- table7 |> dplyr::select(all_of(params$keep2))
```

## Mutate columns

```{r mutate7, eval = params$eval_tb7}
...
```{r ranges7, eval = params$eval_tb7}
table7 <- bwu::gpluck_make_score_ranges(table = table7, test_type = "npsych_test")
```

## Narrow subdomains

```{r narrow7, eval = params$eval_tb7}
table7 <-
  table7 %>%
  dplyr::mutate(
    narrow = dplyr::case_when(
      scale == "Digit Span Forward" ~ "Attention Span",
...
```{r result7, eval = params$eval_tb7}
table7 <-
  table7 %>%
  dplyr::mutate(
    result = glue::glue(
...
```{r relocate7, eval = params$eval_tb7}

...
table1 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      scale == "Full Scale IQ (FSIQ)" ~ "General intelligence",
      scale == "General Ability (GAI)" ~ "General intelligence with reduced influences of working memory and processing speed",
      scale == "Verbal Comprehension (VCI)" ~ "An estimate of Crystallized Intelligence (Gc)",
      scale == "Perceptual Reasoning (PRI)" ~ "Fluid Reasoning (Gf)",
      scale == "Working Memory (WMI)" ~ "An estimate of Short-Term Working Memory (Gsm), which refers to the ability to hold information in immediate awareness and then manipulate or transform it in some way within a few seconds",
      scale == "Processing Speed (PSI)" ~ "Processing Speed (Gs) performance (i.e., the efficiency of cognitive processing or speed of mental activity, especially when under pressure to maintain attention and concentration)",
  ...
  dplyr::mutate(
    result = dplyr::case_when(
      ...
      scale == "Verbal Comprehension (VCI)" ~ glue::glue(
        "{description} was classified as {range} and ranked at the {percentile}th percentile, indicating performance as good as or better than {percentile}% of same age peers from the general population.\n"
      ),
      scale == "Perceptual Reasoning (PRI)" ~ glue::glue(
        "{description} was classified as {range} and ranked at the {percentile}th percentile, indicating performance as good as or better than {percentile}% of same age peers from the general population.\n"
      ),
      scale == "Working Memory (WMI)" ~ glue::glue(
        "{description}, was a relative [strength/weakness], falling in the {range} range.\n"
      ),
      scale == "Processing Speed (PSI)" ~ glue::glue(
        "{description}, was {range} and a relative [strength/weakness], higher than {percentile}% of same-age peers.\n"
      ),
...
table4 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      scale == "Digit Span" ~ "{patient}'s ability to register, maintain, and manipulate auditory information",
      scale == "Arithmetic" ~ "{patient}'s ability to solve math word problems in working memory",
      scale == "Letter-Number Sequencing" ~ "{patient}'s ability to register, maintain, manipulate, and resequence series of numbers and letters in working memory",
      TRUE ~ as.character(description)
    )
  )
```
...
table6 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      scale == "Block Design No Time Bonus" ~ "Untimed Block Design performance",
      TRUE ~ as.character(description)
    )
  )
```
...
table7 <-
  ...
  dplyr::mutate(
    description = dplyr::case_when(
      scale == "Digit Span Forward" ~ "{patient}'s ability to register, maintain, and manipulate auditory information",
      scale == "Digit Span Backward" ~ "Verbal working memory",
      scale == "Digit Span Sequencing" ~ "Verbal working memory and sequencing",
      scale == "Longest Digit Span Forward" ~ "Auditory attentional capacity",
      scale == "Longest Digit Span Backward" ~ "Working memory for orally presented information",
      scale == "Longest Digit Span Sequence" ~ "Recall of digits sequences/resequencing",
...
```{r write}
readr::write_csv(wais4, here::here(patient, "csv", "wais4.csv"), col_names = TRUE, na = "", delim = ", ")
```

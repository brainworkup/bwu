...
`r params$fname` is a (ref:age)-year-old youngster who presents for
...
provide diagnostic clarification and to strategize how to best prepare
`r params$fname` moving forward in middle school and high school. This report is
based on a review of available medical records and information gathered across
multiple days of evaluation. Results and recommendations were discussed with
`r params$fname`'s parents, Dad and Mom, `r params$lname`, on the last of these
evaluation dates during interactive feedback, and with `r params$fname` a week
later.

## Background

`r params$fname`'s parents are concerned that he has trouble focusing and
interpreting information at times, and with abstract thinking and seeing the
"bigger picture." `r params$he2` has trouble with reading comprehension and math
word problems. If he finds the main idea of what he is reading, he has trouble
finding the supporting ideas. `r params$he2` is not good with time management
but is learning. He has a tendency toward procrastination, and a lack of focus
and comprehension sometimes with his studies. Part of the impetus for the
current evaluation is to determine, if possible, the reasons as to why
`r params$fname` struggled on his ISEE middle school placement exams this year
given his otherwise strong academic record. `r params$his2` parents noted via
his tutor that he struggles with anything related to deep reading. These issues
were not discovered by his teachers.
...
`r params$fname` lives with `r params$his` parents Dad and Mom, and his sister
(age x) and brother (age x) in Los Angeles, CA. Dr. `r params$lname` is a X and
Mrs. `r params$lname` X. When asked about major changes in the family situation
or family stressors over the past year, `r params$fname`'s parents reported no
major stressors other than the pandemic. Unremarkable family medical history.

### Academics

`r params$fname` attends sixth grade at XX Elementary School in Los Angeles and
has since kindergarten, always doing reasonably well academically and socially.
He briefly worked with a reading specialist in kindergarten at his teachers
suggestion. XX returned to having students come to campus for in-person
schooling, which was exciting for `r params$fname`. He took placement exams for
middle school a few months ago and did not score as well as expected.
`r params$he2` applied to three highly competitive middle school programs and
recently received acceptance to the XX school.

### Behavioral/Emotional/Social

`r params$fname` is a kind, compassionate, inclusive, and friendly child. He is
athletic and a fierce competitor but humble. He is extremely sensitive and wears
his heart on his sleeve. `r params$he2` is genuine and loyal and a friend to
everyone. At times, he is not always assertive when he probably should be.
`r params$he2` is shy and quiet in class and does not always participate or
express himself. Parents worry he lacks self-confidence in himself and in his
abilities. He gets easily frustrated when he can't do something or is
overwhelmed. He is social and has a number of good, close friends.
`r params$he2` enjoys playing basketball competitively with his peers, and
recreationally at home with his father. He plays tennis, swims, plays piano, and
plays video games for fun. `r params$he2` worked with a psychologist, XX XX,
PhD, in 2019 to get rid of a his nail-biting habit. No other prior therapy or
psychological testing was indicated.
...
neurocog <- neuropsych %>% dplyr::filter(test_type == "npsych_test")
## Create factors
# domain
neurocog <-
  neurocog %>%
...
# Subset neurocognitive data
neurobehav <- neuropsych %>% dplyr::filter(test_type != "npsych_test")
## Create factors
# domain
neurobehav <-
  neurobehav %>%
...
```{r, write-data}
## Write data
readr::write_csv(neuropsych, "neuropsych.csv")
readr::write_csv(neurocog, "neurocog.csv")
readr::write_csv(neurobehav, "neurobehav.csv")
```
...
*high-average*. However, when test results were rescored independent of timing
constraints, `r params$fname` evidenced better performance by about a standard
deviation in areas of fluid reasoning.
...
iq <- 
  ...
  dplyr::filter(!is.na(Score)) %>% 
  dplyr::slice(c(1,2,3,5,6))
```

```{r, iq, double_escape = T, escape = F, fig.fullwidth = TRUE}
kableExtra::kbl(
  iq[, 2:5],
  caption = '(ref:iq)',
  "latex",
...
`r params$fname`'s academic skills were well above age- and grade-expected
levels. Written expression was *exceptionally high*. Reading scores were
*high-average* to *above average*, with strong word reading skills.
`r params$fname`'s composite math score was *high-average*, with particularly
...
dotplot(
  data = acad,
  x = acad$z_mean_sub, # change these if necessary
  y = acad$subdomain,
  domain = "academics"
)
```
...
kableExtra::kbl(
  ...
  kableExtra::add_footnote('(ref:scaled-score)')
```
...
```{r vrb-sub, eval=F, fig.cap = '(ref:vrb)', fig.path = 'figures/npsych2-'}
...
```{r spt-sub, eval=T, fig.cap = '(ref:spt)', fig.path = 'figures/npsych2-'}
# dotplot make
dotplot(
  data = spt,
  x = spt$z_mean_sub,
...
```{r spt-narrow, eval=F, fig.cap = '(ref:spt)', fig.path = 'figures/npsych2-'}
# dotplot make
dotplot(
  data = spt,
  x = spt$z_mean_narrow,
...
```{r}
#slice.exe <- c(5,7,9,11,13:14,15:16,17:18)
executive <-
  make_tibble(
  tibb = executive,
  data = neurocog,
...
(ref:fn-exe) Score = *T* score (M = 50, SD = 10) or Scaled score (M = 10, SD =
3)
...
```{r exe-narrow, eval=F, fig.cap = '(ref:att-exe)', fig.path = 'figures/npsych2-'}
# dotplot
dotplot(
  data = exe,
  x = exe$z_mean_narrow,
...
```{r mtm-sub, eval=F, fig.cap = '(ref:mem)', fig.path = 'figures/npsych2-'}
  # dotplot
dotplot(
  data = mem,
  x = mem$z_mean_sub, # try narrow too
  y = mem$subdomain,
  domain = "memory"
)
```
...
```{r mtr-sub, eval=F, fig.cap='(ref:mtr)', fig.path = 'figures/npsych2-'}
# dotplots
dotplot(
  data = mtr,
  x = mtr$z_mean_sub,
...
1.  Cognitive behavioral therapy (CBT) for anxiety: CBT that relies primarily on
    ...
    essential for helping `r params$fname` work through his challenges with
    perfectionism and the secondary emotional outfall. Provider referrals are
    below (links are clickable).

    -   [Melissa Mose, PhD, LMFT](https://www.melissamosemft.com/)
    -   [Dr. Jenny Yip](https://www.dryip.com/)

2.  Medical. Consultation with a child & adolescent psychiatrist or his
    pediatrician is warranted to treat `r params$fname`'s anxiety with
    medication if necessary or if he does not seem to respond to therapy.
    Selective serotonin reuptake inhibitors (SSRIs) are recommended as safe and
    effective first-line treatments for generalized anxiety and OCD symptoms.
    Provider referrals are below (links are clickable).
...
4.  `r params$fname` and his parents are encouraged to learn more about the
    nature of perfectionism and performance/test anxiety, its symptoms, causes,
    and available treatment options. Recommended sources of information include
    these online resources (links are clickable):
...
1.  Extracurricular activities: Non-academic activities are highly encouraged
    (yet carefully balanced with `r params$his` academic demands) to foster
    continued positive socialization experiences.

## Follow-Up Evaluation

1.  Follow up testing is not indicated at this time unless concerns arise that

...
ln -shfi ~/uscdbox/npsych_reports/2021/LASTNAME-FIRSTNAME/*.pdf ~/npsych.komatufte/$patient/pdf/.
```

```{bash, engine.opts='-l', echo=F, eval=F}
export patient=patient
cp ~/uscdbox/npsych_reports/2021/LASTNAME-FIRSTNAME/*.csv ~/npsych.komatufte/$patient/pre_csv/.
```

```{bash, engine.opts='-l', echo=F, eval=F}
export patient=patient
cp ~/uscdbox/npsych_reports/2021/LASTNAME-FIRSTNAME/*.slk ~/npsych.komatufte/$patient/pre_csv/.
```
...
```{r, eval=F, echo=FALSE}
# add any other pdfs you need
file.pdf <- file_names_csv[1]
```
...
(ref:t-score) Score = _T_ score (M = 50, SD = 10)

(ref:z-score) Score = _z_-score (M = 0, SD = 1)

(ref:zplot) _z_ = -1 (16th percentile), _z_ = 0 (50th percentile), and _z_ = 1
(84th percentile)

(ref:pct) A _percentile_ shows `r params$fname`'s standing relative to peers.
For example, a percentile score of 70 means `r params$fname` performed as well
as or better than 70% of individuals the same age.

(ref:br) A _base rate_ is the prevalence of score in the general population. If
...
(ref:basc3-footnote) Score = _T_ score (M = 50, SD = 10); _Note:_ Adaptive
Skills _T_ scores are reversed

(ref:basc3-fn-srp-adol) Score = _T_ score (M = 50, SD = 10); _Note:_ Personal
Adjustment _T_ scores are reversed

(ref:g) _General intelligence_ refers to an overall capacity to reason, to solve
problems, and to learn useful information.

(ref:gc) _Crystallized intelligence_ refers to the ability to learn and use
language to reason and understand how the world works.

(ref:gf) _Fluid intelligence_ refers to the ability to use logical reasoning to
...
(ref:vrb) _Verbal/language_ refers to the ability to access and apply acquired
...
(ref:spt) _Visuoperception, visuoconstruction, and visuospatial processing_
...
(ref:att-exe) _Attentional_ and _executive functions_ underlie most, if not all,
...
(ref:att) _Attention_ is a basic function that underlies most, if not all,
...
(ref:exe) Measures of _executive functioning_ are used as proxies for behaviors
...
(ref:wm) _Working memory_ is the system responsible for the transient holding
...
(ref:ps) _Processing speed_ refers to the rate at which the brain can
...
(ref:cc) _Cognitive control_ guides behavior by controlling what, when, and how
...
(ref:adhd) _ADHD_ is a neurodevelopmental disorder characterized by symptoms of
...
(ref:dysgraphia) _Dysgraphia_ is an impairment in writing, including problems
...
(ref:odd) _Oppositional defiant disorder (ODD)_ is a tendency to oppose and
...
(ref:dyslexia) _Dyslexia_ means an individual has a neurobiologically based
...
`r params$fname` `r params$lname` is a (ref:age)-year, XX-month-old
right/left-handed male/female with a reported history of XX. He was referred by
Dr Dre, MD, for evaluation of `r params$his` neurocognitive functioning to
update `r params$his` intervention plan. This report is based on a review of
available medical records and information gathered on the following dates:
`r params$doe1`, `r params$doe2`, `r params$doe3`, `r params$doe4`. Intervention
planning was discussed with `r params$fname` and `r params$his`
mother/father/parents/spouse on the last of these dates.
...
`r params$fname` was born full-term following an uncomplicated pregnancy, labor,
and delivery. `r params$he2` was a healthy child until Month 20XX when
`r params$he` sustained `r params$his` first concussion. `r params$he2` had
additional concussions in Month 20XX and Month 20XX. All three were
sports-related and involved brief alterations in consciousness, followed by
headache and concentration problems that resolved within a week. In Month 20XX
`r params$he` sustained `r params$his` latest concussion after hitting
`r params$his` head on a table. `r params$he2` did not lose consciousness, but
`r params$he` experienced increased headaches/migraines and nausea/vomiting.
`r params$he2` missed the entire second quarter of school but was eventually
able to return for half-days. `r params$he2` continued to struggle to attend
school and missed at least 1 day weekly due to migraine.

`r params$his2` medical history also includes attention problems, which were
longstanding but first treated after his 20XX concussion. He now takes Concerta.
However, `r params$his` mother reported ongoing difficulties with
focus/concentration and forgetfulness. She said `r params$he` struggles to
remember tasks he needs to do and forgets to complete and turn in homework. She
also reported that it has been difficult to get `r params$fname` to take his
medications if she is not there to remind him. He has told her that he would
...
vision, sleep, appetite or growth. `r params$he2` denies current headaches or
pain.

### Academics

`r params$fname` is in `r params$educ` at XX School in Los Angeles.
...
diploma. He earned C's, D's, and F's in his classes. He is now taking English
(online course) in summer school. His mother reported that he is frustrated with
his grades, as he has been an A/B student in the past. She said that he has
trouble with daily work as well as quizzes/tests. His teachers have reported
poor participation in class.

### Behavioral/Emotional/Social

`r params$fname`'s mother reported that he has struggled with depression over
the years. He was especially upset at no longer being allowed to play sports
after his previous concussions. She noted that his mood has been better and he
has not been as withdrawn since increasing his dose of Effexor XR. She denied
concerns about him harming himself or others. She expressed concern about his
coping skills. She also said that he has always tended to want things a certain
way, in a certain order. However, this has not caused problems in daily life.
She denied any other anxiety.

`r params$fname` has many friends. He enjoys pick-up basketball and watching
basketball games. His mother reported that he recently got his driver's license
...
`r params$lname` is a X and Mrs. `r params$lname` is a X. Both parents graduated
high school. Recent stressors include a family member's illness. Family mental
health history includes depression.

### Prior Testing

`r params$fname` had a previous neuropsychological evaluation (Month 20XX),
shortly following his 20XX concussion. Results at that time indicated
...
testing he had not taken stimulant medication and reported continuous headache.
...
as percentiles[^1], which are provided in the tables below, and as _z_-scores (M
...
Overall intellectual scores were _Average_. Most index scores were _Average_ to
...
`r params$fname`'s academic skills were at or above age-expected levels. Reading
scores were _Low-Average_ to _High-Average_, with strong word reading skills.
Spelling and written expression were _Low-Average_ to _High-Average_.
`r params$fname`'s composite math score was _Average_, with strong math problem
...
All scores in this area were _Average_ to _Above Average_. An exception was
`r params$his` phonemic word fluency score, which was _Low-Average_ and a
relative weakness. Phonemic fluency is a component of broader phonological
awareness and a core building block to reading development in early childhood.
...
(ref:fn-verbal) Score = Standard score (M = 100, SD = 15), _T_ score (M = 50, SD
...
Scores were again average to high average here, without a clear pattern of
...
(ref:fn-spatial) Score = Standard score (M = 100, SD = 15), _T_ score (M = 50,
...
Brief attention/working memory, directed attention, sustained attention, and
impulse control were all average. His performance on tasks of complex problem
solving and mental flexibility was average to high-average. Scores on timed
tests varied, but tended to be a bit weaker than on untimed tests.
...
This seems to be the case for John, as parent ratings indicated significant
attention problems, even with medication. Both parent ratings and John's own
ratings of his executive skills indicated difficulties with working memory and
...
(ref:fn-exe) Score = Standard score (M = 100, SD = 15), _T_ score (M = 50, SD =
10), or Scaled score (M = 10, SD = 3)
...
Learning and recall of rote verbal information (word list) was _Average_
overall, and `r params$fname` showed a variable learning curve. Recall of the
list after a brief delay was _Average_. Recall after a longer delay was
_Low-Average_ secondary to `r params$his` variable learning curve resulting in
...
information (short story) was _High-Average_ for immediate free recall. Recall
of visual material (abstract shapes) was _High-Average_. Recall after a long
delay was _Above Average_. Reconstructing a complex/abstract visual design from
...
(ref:fn-memory) Score = _T_ score (M = 50, SD = 10) or Scaled score (M = 10, SD
= 3)
...
All scores in this area were _Average_ to _Exceptionally High_. Fine motor speed
...
accuracy (i.e., graphomotor control) was _Average_. Gross motor functioning
...
(ref:fn-motor) Score = _T_ score (M = 50, SD = 10) or Scaled score (M = 10, SD = 3)
...
daily living. His mother also noted mild concerns with depression. Teacher
...
(ref:domain) _Key Findings:_ Academic and intellectual skills are
...
_specific-learning disorder (SLD) with impairment in mathematics_. **_The
...
processing and processing speed._**
...
depression symptoms are diagnostically consistent with _dysthymia_, along with
...
3.  School Plan. Consider providing school supports under a 504 Plan or Service
    Learning Plan, given John's history of multiple concussions and longstanding
    diagnosis of ADHD. His parents are encouraged to share a copy of this report
    with his school to develop such a plan.

4.  Attention.

    a. Allowhimtotaketestsinaquietlocation,separatefromclassmates,tominimizedistractions.
    b. Provide preferential seating (i.e. close to the teacher and board, away
    from talkative/disruptive students or open doors/windows).
    c. Have him focus on one task at a time. Repeat and/or clarify directions
    as needed.
    d. Check in with him periodically to make sure he is on-task and knows what
    he is supposed to do.
    e. Provide reminders for work completion.
    f. Prompt him to use his homework agenda/planner to record assignments and
    due dates.

5.  Organization/Study Skills. Consider offering a supervised study hall that
    has an explicit focus on building his use of organization and study skills.
    Use his actual classwork as material, initially making sure he understands
    key organization and study skills (e.g. use of an agenda, breaking down
    assignments), then prompting him to use them, fading the prompts over time.
    Author and Author's book 'Book Title Omitted to Avoid Commercial
    Endorsement' offers a nicely coherent and practical approach. Also reinforce
    the concepts recommended in the next section.
...
| **Score range**    | **Standard score** | **_T_** **score** | **Scaled score** | **\*z\*\*\***-score\*\* | **Percentile (‰)** |
| :----------------- | :----------------: | :---------------: | :--------------: | :---------------------: | :----------------: |

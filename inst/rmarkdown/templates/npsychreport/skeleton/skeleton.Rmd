...
```{r, eval=F}
# plot domain scores as z-scores; unique to `r params$fname`
g1 <- neurocog %>% dplyr::filter(domain == "Intelligence") %>% dplyr::filter(!is.na(score)) 
g2 <- neurocog %>% dplyr::filter(domain == "Visual Perception/Construction") %>% dplyr::filter(!is.na(score)) 
g3 <- neurocog %>% dplyr::filter(domain == "Attention/Executive") %>% dplyr::filter(!is.na(score)) 
g <- dplyr::bind_rows(g1,g2,g3) %>% dplyr::filter(!is.na(score)) %>% dplyr::arrange(test_name) %>% dplyr::slice(c(8,17,20:28))
```
...
overall, and `r params$fname` showed a variable learning curve[^3]. Recall of
...
[^3]: He had recalled as many as 14/15 words correctly during the learning

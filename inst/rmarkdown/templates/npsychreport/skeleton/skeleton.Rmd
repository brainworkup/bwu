...
```{r, eval=F, echo=FALSE}
## set file path # this might be useless
path_txt = here::here(patient, "pre_csv")
file_names_txt <- dir(path_txt, pattern = ".txt")
file_names_txt
```
...
```{r, eval=F, echo=FALSE}
# add any other pdfs/csv/etc files needed
file.csv <- file_names_csv[1]
```
...
```{r 02-make-dataframe-iq, eval=T, double_escape = T, escape = F}
```
...
```{r 03-make-kable-iq, eval=T, double_escape = T, escape = F, fig.fullwidth = TRUE}
```
...
```{r 04-make-dataframe-g, eval=T, double_escape = T, escape = F}
```
...
```{r 05-plot-iq-narrow, eval=T, double_escape = T, escape = F, fig.cap = '(ref:g) (ref:gc) (ref:gf)', fig.path = 'figures/npsych2-'}
```
...
```{r 03-make-kable-academics, eval=T, double_escape = T, escape = F, fig.fullwidth = TRUE}
```
...
```{r 05-plot-acad-narrow, eval=T, double_escape = T, escape = F, fig.cap = '(ref:academics)', fig.path = 'figures/npsych2-'}
```
...
```{r 02-make-dataframe-verbal, eval=T, double_escape = T, escape = F}
```
...
```{r 04-make-dataframe-vrb, eval=T, double_escape = T, escape = F}
```
...
```{r 05-plot-vrb-narrow, eval = TRUE, fig.cap = '(ref:vrb)', double_escape = T, escape = F}
```
...
```{r 02-make-dataframe-executive, eval=T, double_escape = T, escape = F}
```
...
```{r 04-make-dataframe-exe, eval=T, double_escape = T, escape = F}
```
...
```{r 05-plot-exe-sub, eval=T, fig.cap = '(ref:att-exe)', double_escape = T, escape = F}
```
...
overall, and `r params$fname` showed a good learning curve. Recall of the list
after a brief delay was *Average*. Recall after a longer delay was *Low-Average*
secondary to `r params$his` variable learning curve resulting in disorganized
initial encoding of the words, which was `r params$his` only memory score
...
```{r 02-make-dataframe-memory, eval=T, double_escape = T, escape = F}
```
...
```{r 05-plot-mem-narrow, eval=T, fig.cap = '(ref:mem)', double_escape = T, escape = F}
```
...
(ref:fn-motor) Score = *T* (M = 50, SD = 10) or Scaled (M = 10, SD = 3) scores

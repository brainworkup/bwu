...
(ref:brown-efa-self) Brown Executive Function/Attention Scale (EF/A), Self

(ref:brown-efa-teacher) Brown Executive Function/Attention Scales (EF/A), Teacher
...
(ref:standard-score) Score = Standard (M = 100, SD = 15)

(ref:scaled-score) Score = Scaled (M = 10, SD = 3)

(ref:t-score) Score = *T* (M = 50, SD = 10)

(ref:z-score) Score = *z* (M = 0, SD = 1)
...
(ref:br) A *base rate* is the prevalence of score in the general population. If a score is rare, defined as a base rate of less than 10%, it is considered *clinically significant*, and that area of functioning is probably causing problems in daily functioning.
...
(ref:soc-cogl) *Social cognition* measures are used to gauge aspects of social-emotional processing and social aptitude (sometimes referred to as emotional intelligence), which are important facets of how individuals process social information about other children, adults, groups, and social contexts.
...
```{r bobs, results='asis'}
cat(readLines("02.0.bobs.md"), sep = "\n")
```
...
```{r, iq, results='asis', eval=T}
cat(readLines("02.1.1.iq.md"), sep = "\n")
```
...
```{r, acad, results='asis', eval=T}
cat(readLines("02.1.2.acad.md"), sep = "\n")
```
...
(ref:fn-verbal) Score = Standard (M = 100, SD = 15), *T* (M = 50, SD = 10), or Scaled (M = 10, SD = 3)

```{r, vrb, results='asis', eval=T}
cat(readLines("02.1.3.vrb.md"), sep = "\n")
```
...
```{r, spt, results='asis', eval=T}
cat(readLines("02.1.4.spt.md"), sep = "\n")
```
...
(ref:fn-exe) Score = Standard (M = 100, SD = 15), *T* (M = 50, SD = 10), or Scaled (M 
= 10, SD = 3)

```{r, exe, results='asis', eval=T}
cat(readLines("02.1.5.exe.md"), sep = "\n")
```
...
(ref:fn-memory) Score = Standard (M = 100, SD = 15), *T* (M = 50, SD = 10), or Scaled (M = 10, SD = 3)

```{r, mem, results='asis', eval=T}
cat(readLines("02.1.6.mem.md"), sep = "\n")
```
...
```{r, mtr, results='asis', eval=T}
cat(readLines("02.1.7.mtr.md"), sep = "\n")
```
...
(ref:fn-soc) Scaled (M = 10, SD = 3)

```{r, soc, results='asis', eval=T}
cat(readLines("02.1.8.soc.md"), sep = "\n")
```
...
```{r 05-plot-social-subdomain, eval=T, double_escape=T, escape=F, fig.cap='(ref:social)'}
```

```{r 06-plot-social-narrow, eval=T, double_escape=T, escape=F, fig.cap='(ref:social)'}
```
...
```{r, brown, results='asis', eval=T}
cat(readLines("02.2.brown.md"), sep = "\n")
```
...
```{r, caars, results='asis', eval=T}
cat(readLines("02.2.caars.md"), sep = "\n")
```
...
```{r, basc, results='asis', eval=T}
cat(readLines("02.2.basc.md"), sep = "\n")
```
...
```{r 05-plot-basc3-prs-preschool-subdomain, eval=F, fig.cap = 'Self-reported executive and attentional functioning related to ADHD in everyday life.', double_escape=T, escape=F}
```

```{r 06-plot-basc3-prs-preschool-narrow, eval=T, fig.cap = 'Self-reported executive and attentional functioning related to ADHD in everyday life.', double_escape=T, escape=F}
```

\newpage

```{r, pai, results='asis', eval=T}
cat(readLines("02.2.pai.md"), sep = "\n")
```
...
```{r 05-plot-pai-subdomain, eval=T, fig.margin=TRUE, fig.env='marginfigure', fig.cap='PAI Clinical Domains. Depression and somatic/health complaints are elevated to a clinically signficant degree and need to be prioritized as part of the treatment plan.', fig.height=5, out.width='.5\\textwidth', out.extra='', fig.pos=''}
```

```{r 06-plot-pai-narrow, eval=F, fig.margin=TRUE, fig.env='marginfigure', fig.cap='PAI Clinical Domains. Depression and somatic/health complaints are elevated to a clinically signficant degree and need to be prioritized as part of the treatment plan.', fig.height=5, out.width='.5\\textwidth', out.extra='', fig.pos=''}
```
...
(ref:domain) *Key Findings:* Verbal skills are significantly stronger than nonverbal/spatial processing skills. Attentional fluency (i.e., working memory and processing speed) and abstract reasoning are areas of weakness. Memory continues to evidence a decline from premorbid levels. *Note:* *z* = 0 (mean) ± 1 (sd)
...
```{r, sirf, results='asis'}
cat(readLines("03.0.sirf.md"), sep = "\n")
```
...
```{r results='asis'}
cat(readLines("03.2.telehealth_statement.md"), sep = "\n")
```

...
```{r, academics, results='asis', eval=T}
...
(ref:fn-verbal) Standard (M = 100, SD = 15), *T* (M = 50, SD = 10), or Scaled (M = 10, SD = 3) scores

```{r, verbal, results='asis', eval=T}
...
```{r, spatial, results='asis', eval=T}
cat(readLines("nb_spatial.md"), sep = "\n")
```
...
```{r, executive, results='asis', eval=T}
...
(ref:fn-memory) Standard (M = 100, SD = 15), *T* (M = 50, SD = 10), or Scaled (M = 10, SD = 3) scores

```{r, memory, results='asis', eval=T}
...
(ref:fn-motor) *T* (M = 50, SD = 10) or Scaled (M = 10, SD = 3) scores

```{r, motor, results='asis', eval=T}
...
```{r 05-plot-social-subdomain, eval=T, double_escape=T, escape=F, fig.cap='(ref:soc-cog)'}
```
...
```{r 05-plot-pai-subdomain, eval=F, fig.margin=TRUE, fig.env='marginfigure', fig.cap='PAI Clinical Domains. Depression and somatic/health complaints are elevated to a clinically signficant degree and need to be prioritized as part of the treatment plan.', fig.height=5, out.width='.5\\textwidth', out.extra='', fig.pos=''}
```

...
This report is based on a review of available medical records and information
gathered across a 1-hour neurobehavioral exam conducted with the patient via
telemedicine on `r params$doe1`.
...
Patient during interactive feedback on the final day of the evaluation. -->
...
the years. `r params$he_she_cap` also said that `r params$he_she` has always
tended to want things a certain way, in a certain order. However, this has not
caused problems in daily life. `r params$he_she_cap` denied any other anxiety.

`r params$first_name` has many friends. `r params$he_she_cap` enjoys x, y z.

### Academics

`r params$first_name` is currently a student at USC.
...
*Orientation:* Patient is oriented to time, place, and person. *Attention and
Concentration:* Patient is able to sustain attention and concentration throughout
the interview. There are no signs of distractibility or difficulties with
task-switching. *Language:* Patient exhibits fluent and clear speech, with
appropriate syntax and grammar. There are no signs of aphasia,
dysarthria, or anomia. Patient is able to comprehend and follow verbal
instructions. *Memory:* Patient is able to recall recent and remote events without
difficulty. There are no signs of amnesia or confabulation. *Executive Functioning:* Patient demonstrates intact executive functioning, with the ability to plan, initiate, and complete tasks without difficulty. There are no signs of impulsivity, perseveration, or difficulties with decision-making. *Motor Functioning:* Patient exhibits normal
motor functioning with no signs of ataxia or tremors. Gait and posture are
normal. *Sensory Functioning:* Patient exhibits normal sensory functioning with no
signs of numbness, tingling, or paresthesias. *Mood and Affect:* Patient's mood
is euthymic and affect is appropriate throughout the interview. There are no signs of anxiety, depression, or mania.

## Summary/Impression

Overall, this brief neurobehavioral status exam can be used as a starting point
for a more comprehensive neuropsychological evaluation. It provides an overview
of the patient's cognitive, behavioral, and emotional functioning and can
help guide further assessment and treatment planning.

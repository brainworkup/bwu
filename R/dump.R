# domain
gg_plot_dots <- function(neurocog, z_mean_dom, domain, z) {
  ggplot() +
...
# subdomain narrow
ggplot() +
  geom_segment(data = neurocog, aes(x = z_mean_narrow, y = reorder(subdomain_narrow, z_mean_narrow), xend = 0, yend = domain), size = 1) +

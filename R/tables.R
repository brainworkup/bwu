...
#' @title Make Table Using `gt` Package for Neurocognitive Domains (Version 2)
#' @description Create a table of domain counts using dplyr and gt packages.
#' This function allows for specifying which columns from the data should be included in the 
#' final table.
#' 
#' @importFrom dplyr across mutate group_by summarize arrange select if_else any_of all_of
#' @importFrom gt gt cols_label tab_stub_indent tab_header sub_missing
#'   tab_options cols_align tab_source_note gtsave tab_style tab_stubhead
#'   tab_caption tab_spanner cell_text cells_source_notes md tab_footnote
#'   opt_vertical_padding cells_row_groups
...
#' @param vertical_padding Vertical padding.
#' @param multiline Multiline footnotes, Default = TRUE.
#' @param table_vars Character vector of columns to include in the table. 
#' Defaults to `c("score", "percentile", "range")`.
#' @param ... Additional arguments to be passed to the function.
#' @return A formatted table with domain counts.
#' @rdname tbl_gt2
#' @export
tbl_gt2 <-
  function(data,
           ...
           table_vars = c("score", "percentile", "range")) { 
    # Create data counts
    data_counts <- data |>
      dplyr::select(test_name, scale, all_of(table_vars)) |>
      dplyr::mutate(across(any_of(c("score", "percentile")), ~ tidyr::replace_na(., replace = 0)))
    ...
    table <- data_counts |>
      ...
      cols_align(
        ...
        columns = any_of(c("score", "percentile", "range"))
      )
